...
import { farmerData, farmerFAQs, farmerSupportContact } from "../../mockData/farmer";
          ...
          {[["faq", "FAQs"], ["contact", "Contact"]].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className={`flex-1 py-2.5 rounded-xl font-sans font-semibold text-sm transition-all ${
                tab === key ? "bg-brand-green text-white shadow-sm" : "text-brand-text-secondary"
              }`}>
              {label}
            </button>
          ))}
          ...
          <>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <a href="tel:08133905285"
                className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl bg-brand-green text-white font-sans font-semibold text-sm">
