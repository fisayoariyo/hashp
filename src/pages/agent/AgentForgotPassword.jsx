...
export default function AgentForgotPassword() {
  ...
  const formBody = (
    <div className="space-y-4 w-full">
      ...
      <p className="font-sans text-xs text-brand-text-muted">Demo: use any phone number, then OTP <strong>1234</strong> on the next step.</p>
    </div>
  );

  const actions = (
    <div className="space-y-3 w-full">
      <button type="button" onClick={handleContinue} disabled={loading || phone.replace(/\D/g, "").length < 10} className="btn-primary w-full">
