...
import {
  resolveFarmerAgentUpgradeScreen,
  writeStoredFarmerUpgradeStatus,
} from "../../utils/agentStatus";

function readFarmerKey(dashboard) {
  const farmer = dashboard?.farmer;
  const session = getFarmerSession();
  return (
    (farmer && typeof farmer === "object" && (farmer.farmer_id || farmer.id)) ||
    session?.farmerUserId ||
    ""
  );
}
...
export default function FarmerSettings() {
  ...
  useEffect(() => {
    ...
    const farmerKey = readFarmerKey(dashboard);
    if (farmerKey) {
      writeStoredFarmerUpgradeStatus(farmerKey, queryStatus);
    }
    setScreen(queryStatus);
  }, [location.search, dashboard]);

  const farmerKey = useMemo(() => readFarmerKey(dashboard), [dashboard]);

  const agentUpgradeScreen = useMemo(
    () => resolveFarmerAgentUpgradeScreen(dashboard, farmerKey),
    [dashboard, farmerKey],
  );
  ...
  const persistUpgradeScreen = (upgradeScreen) => {
    writeStoredFarmerUpgradeStatus(farmerKey, upgradeScreen);
  };

  const openAgentModeFlow = () => {
    setModeMenuOpen(false);

    if (getAgentAccessToken()) {
      persistUpgradeScreen("verified");
      handleSwitchToAgent();
      return;
    }

    if (agentUpgradeScreen === "verified") {
      handleSwitchToAgent();
      return;
    }
    if (agentUpgradeScreen === "under_review") {
      setScreen("under_review");
      return;
    }
    if (agentUpgradeScreen === "failed") {
      setScreen("failed");
      return;
    }
    setScreen("form");
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      persistUpgradeScreen("under_review");
  ...
  const handleRefreshStatus = async () => {
    try {
      ...
      setDashboard(payload);
      const key = readFarmerKey(payload);
      const upgradeScreen = resolveFarmerAgentUpgradeScreen(payload, key);
      if (upgradeScreen === "verified") {
        setScreen("verified");
      } else if (upgradeScreen === "failed") {
        setScreen("failed");
      }
    } catch {
    ...
    if (screen === "verified") {
      return (
        <StatusView
          ...
          onSecondary={() => setScreen("settings")}
          onBack={handleBack}
        />
      );
    }
    if (screen === "failed") {
      return (
        <StatusView
          ...
          onPrimary={() => {
            writeStoredFarmerUpgradeStatus(farmerKey, "none");
            setScreen("form");
          }}
